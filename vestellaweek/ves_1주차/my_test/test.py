...
result = eliminate_light(img)

origin = cv2.resize(result, dsize=(640, 480), interpolation=cv2.INTER_AREA)
origin_image = cv2.cvtColor(origin, cv2.COLOR_BGR2HSV)
param = {
    'image' : origin_image
}

mask = cv2.inRange(origin_image, (10,0,40),(190,40,100))
...
origin_test = cv2.bitwise_and(mask, origin)
