...
import numpy
import time
from moving_check import moving_check

model = torch.hub.load('ultralytics/yolov5', 'yolov5m',
                            device='cuda:0' if torch.cuda.is_available() else 'cpu')
model.classes = [2] #자동차만
cap = cv2.VideoCapture('/mnt/c/Users/tom41/OneDrive/Desktop/dataset_videos/CCTV17_crop_3.mp4')
...
while(True):
    ...
    check = moving_check(frame,time_check)
    while(check == False):
        time_check = 0
    else:
        time_check += 1
